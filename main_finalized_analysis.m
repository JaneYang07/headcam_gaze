...
% Hypothesis 1: Whether or not an object is salient or not in parent's
% view, as long as either child or parent is holding the object, it will
% lead to parent's attention.

% Results needed:
%               P(parent’s attention | parent vis saliency & parent inhand) 
%               P(parent’s attention | no parent vis saliency & parent inhand)
%               P(parent’s attention | parent vis saliency & child inhand)
%               P(parent’s attention | no parent vis saliency & child inhand)
obj_size_child = 0.04;
...
index_both_inhand = intersect(index_parent_inhand,index_child_inhand);
index_child_inhand = setdiff(index_child_inhand,index_both_inhand); % c inhand only
index_parent_inhand = setdiff(index_parent_inhand,index_both_inhand); % p inhand only
...
for i = 1:numel(sub_list)
    ...
    sub_index_both_inhand = intersect(sub_index_parent_inhand,sub_index_child_inhand);
    sub_index_parent_inhand = setdiff(sub_index_parent_inhand,sub_index_both_inhand);
    sub_index_child_inhand = setdiff(sub_index_child_inhand,sub_index_both_inhand);
